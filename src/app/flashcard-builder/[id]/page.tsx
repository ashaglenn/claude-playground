...
import {
  ...
  flashcardContentToBuilderState,
...
export default function EditFlashcardBuilderPage() {
  const params = useParams()
  const flashcardId = params.id as string
  const [state, setState] = useState<FlashcardBuilderState | null>(null)
  ...
  const loadFlashcards = async () => {
    ...
    const builderState = flashcardContentToBuilderState(
      data.game_content as FlashcardContent,
      data.title
    )
  ...
  const updateDescription = (description: string) => {
    setState(prev => prev ? { ...prev, description } : prev)
  }
  ...
  const removeCard = (index: number) => {
    setState(prev => {
      if (!prev || prev.cards.length <= 1) return prev
      return {
        ...prev,
